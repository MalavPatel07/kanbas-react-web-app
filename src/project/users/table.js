...
function UserTable() {
  ...
  const deleteUser = async (user) => {
    try {
      const status = await client.deleteUser(user);
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user._id}>
              ...
              <td>{user.lastName}</td>
              <td>
                <BsTrash3Fill onClick={() => deleteUser(user)} className="text-danger fs-1 text"/>
                </td>
                <td>
                <BsPencil onClick={() => selectUser(user)} className="text-success fs-1 text"/>
                </td>
            </tr>))}
        </tbody>
      </table>
